...
import {
    findSzervezoIDNevvel, findSzervezoNev
...
export async function findallSzervezoNevReszfeladatokon (reszfeladatokSzervezoID){
    let reszfeladatokSzervezoNevek = [];
    let reszfeladatSzervezok;
    let jelenlegiReszfeladatSzervezok = []
    await Promise.all(reszfeladatokSzervezoID.map(async (jelenlegiReszfeladatID) => {
        jelenlegiReszfeladatSzervezok = []
        await Promise.all(jelenlegiReszfeladatID.map(async (jelenlegiSzervezoID) => {
            // let jelenlegiSzervezoNev = (await findSzervezoNev(jelenlegiSzervezoID.szervezoID))[0][0]
            // jelenlegiReszfeladatSzervezok.push(jelenlegiSzervezoNev)
        }));
        // reszfeladatokSzervezoNevek = reszfeladatokSzervezoNevek.concat(jelenlegiReszfeladatSzervezok)
        reszfeladatokSzervezoNevek.push.apply(reszfeladatokSzervezoNevek,[jelenlegiReszfeladatSzervezok])
    }));
    return reszfeladatokSzervezoNevek;
}

export async function findReszfeladatonDolgozik (reszfeladatID, szervezoID){
    let jelenlegiSzervezo = await connectionPool.query(`select RendezvenyReszfeladatokSzervezok.reszfeladatSzervezoID
          from RendezvenyReszfeladatokSzervezok
...
export async function insertSzervezokReszfeladatokra (csatlakozasVagyKilepes, szervezoID, reszfeladatID){
    let beszurtRendezvenySzervezo;
...
export async function reszfeladatSzervezokNevei (reszfeldatID){
    let reszfeladatSzervezoi = await connectionPool.query(
        `select DISTINCT Szervezo.szervezoNev from RendezvenyReszfeladatokSzervezok 
